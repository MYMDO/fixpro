...
from .protocols import (
    SYNC_BYTE, USB_MAX_PACKET_SIZE,
    CMD_PING, CMD_GET_INFO, CMD_RESET, CMD_GET_STATUS,
    CMD_SPI_INIT, CMD_SPI_DEINIT, CMD_SPI_TRANSFER, CMD_SPI_WRITE, CMD_SPI_READ,
    CMD_I2C_INIT, CMD_I2C_WRITE, CMD_I2C_READ, CMD_I2C_SCAN,
    CMD_JTAG_INIT, CMD_JTAG_DEINIT, CMD_JTAG_RESET, CMD_JTAG_SHIFT, CMD_JTAG_READ_IDCODE,
    CMD_SWD_INIT, CMD_SWD_DEINIT, CMD_SWD_RESET, CMD_SWD_READ, CMD_SWD_WRITE,
    CMD_UPDI_INIT, CMD_UPDI_DEINIT, CMD_UPDI_RESET, CMD_UPDI_READ_INFO,
    CMD_UPDI_READ_FLASH, CMD_UPDI_WRITE_FLASH, CMD_UPDI_ERASE_CHIP,
    CMD_UPDI_READ_FUSE, CMD_UPDI_WRITE_FUSE,
    CMD_1WIRE_INIT, CMD_1WIRE_DEINIT, CMD_1WIRE_RESET, CMD_1WIRE_SEARCH,
    CMD_1WIRE_READ_ROM, CMD_1WIRE_READ_TEMP,
    STAT_OK, STAT_OK_WITH_DATA, STAT_ERROR_TIMEOUT
)
...
class FiXProDevice:
        ...
        try:
            while True:
                ...
                if byte == bytes([SYNC_BYTE]):
                    break
                if timeout and (time.time() - self.serial._start_time if hasattr(self.serial, '_start_time') else True):
                    if not byte:
    ...
    def _execute_with_status(self, cmd: int, data: bytes = b'', timeout: float = 1.0) -> Tuple[int, bytes]:
