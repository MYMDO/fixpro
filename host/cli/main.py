            ...
            with open(avr_db_path, 'r') as f:
                ...
                for chip in data.get('chips', []):
                    if 'signature' in chip:
                        sig = tuple(int(x, 16) if isinstance(x, str) else x for x in chip['signature'])
                        AVR_DB[sig] = chip
        except Exception as e:
...
def cmd_erase(args) -> int:
    ...
    with FiXProDevice() as dev:
        ...
        if hasattr(args, 'sector') and args.sector is not None:
            ...
            dev.spi_transfer(bytes([0x20, (address >> 16) & 0xFF, (address >> 8) & 0xFF, address & 0xFF]))
...
def cmd_info(args) -> int:
    ...
    with FiXProDevice() as dev:
        ...
        if info:
            ...
            print("  Capabilities:")
            if caps & CAP_SPI: print("    - SPI")
            if caps & CAP_I2C: print("    - I2C")
            if caps & CAP_JTAG: print("    - JTAG")
            if caps & CAP_SWD: print("    - SWD")
            if caps & CAP_UPDI: print("    - UPDI")
            if caps & CAP_1WIRE: print("    - 1-Wire")
            if caps & CAP_FLASH_READ: print("    - Flash Read")
            if caps & CAP_FLASH_WRITE: print("    - Flash Write")
...
def cmd_list_chips(args) -> int:
    ...
    if chip_type in ('spi', 'all'):
        ...
        for key, chip in sorted(SPI_FLASH_DB.items(), key=lambda x: x[1].get('name', '')):
            ...
            print(f"  {chip.get('name', 'Unknown'):<25} {size:>6.1f} MB  {mfg:<15} JEDEC: {jedec_str}")
...
Examples:
  ...
  fixpro list-chips --type mcu
    ...
    list_parser.add_argument('--type', choices=['spi', 'eeprom', 'mcu', 'avr', 'all'], default='all')
